	...
	public class Program
	{
		...
		public static bool Quantize(String src,String dst)
		{
			...
			using(Bitmap bm = (Bitmap)Image.FromFile(f))
			{
				...
				{
					Console.WriteLine("Screenshot for " + name + " has wrong size. It must be 64x64 pixels");
					return false;
				}
					...
					for (int xx = 0; xx < tilesx; ++xx)
					{
						...
						for (int y = 0; y < 8; ++y)
						{
							for (int x = 0; x < 8; ++x)
							{
								...
								tilepce[2 * y + 0] |= (byte)(b1 << (7 - x));
								tilepce[2 * y + 1] |= (byte)(b2 << (7 - x));

								tilepce[2 * y + 16 + 0] |= (byte)(b3 << (7 - x));
								tilepce[2 * y + 16 + 1] |= (byte)(b4 << (7 - x));
							}
						}
